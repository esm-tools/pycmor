...
class CMORizer:
    ...
    def __init__(
        ...
        self._pycmor_cfg = PyCMORConfigManager.from_pycmor_cfg(pycmor_cfg or {})
        ...
        if self.cmor_version not in self._SUPPORTED_CMOR_VERSIONS:
            ...
            logger.error(f"Supported versions are {self._SUPPORTED_CMOR}")
        ...
        pycmor_config = PyCMORConfig()
        # NOTE(PG): This variable is for demonstration purposes:
        _pycmor_config_dict = {}
        for namespace, key, value, option in get_runtime_config(
            self._pycmor_cfg, pycmor_config
        ):
            full_key = generate_uppercase_key(key, namespace)
            _pycmor_config_dict[full_key] = value
        logger.info(yaml.dump(_pycmor_config_dict))
        # Avoid confusion:
        del pycmor_config
        logger.info(80 * "#")
        ################################################################################
        ...
        #           out the variable _pycmor_config_dict to see EVERYTHING that is
    ...
    def _post_init_create_dask_cluster(self):
        ...
        cluster_name = self._pycmor_cfg("dask_cluster")
        ClusterClass = CLUSTER_MAPPINGS[cluster_name]
        self._cluster = ClusterClass()
        set_dashboard_link(self._cluster)
        cluster_scaling_mode = self._pycmor_cfg.get(
            "dask_cluster_scaling_mode", "adapt"
        )
        if cluster_scaling_mode == "adapt":
            if CLUSTER_ADAPT_SUPPORT[cluster_name]:
                min_jobs = self._pycmor_cfg.get("dask_cluster_scaling_minimum_jobs", 1)
                max_jobs = self._pycmor_cfg.get("dask_cluster_scaling_maximum_jobs", 10)
            ...
            if CLUSTER_SCALE_SUPPORT[cluster_name]:
                jobs = self._pycmor_cfg.get("dask_cluster_scaling_fixed_jobs", 5)
                self._cluster.scale(jobs=jobs)
            else:
                logger.warning(f"{self._cluster} does not support fixed scaing")
        else:
            raise ValueError(
                "You need to specify adapt or fixed for pycmor.dask_cluster_scaling_mode"
            )
        ...
        if self._pycmor_cfg.get("enable_flox", True):
            dask_extras += 1
    ...
    def find_matching_rule(
        ...
        if len(matches) == 0:
            ...
            if self._pycmor_cfg.get("raise_on_no_rule", False):
                raise ValueError(msg)
            elif self._pycmor_cfg.get("warn_on_no_rule", False):
                logger.warning(msg)
            return None
        if len(matches) > 1:
            msg = f"Need only one rule to match to {data_request_variable}. Found {len(matches)}."
            if self._pycmor_cfg.get("raise_on_multiple_rules", True):
                raise ValueError(msg)
            else:
                logger.critical(msg)
                logger.critical(
                    ...
                    >> pycmor_cfg['raise_on_multiple_rules'] = False <<
                    """
                )
        ...
        for p in self.pipelines:
            if isinstance(p, Pipeline):
                ...
                p["workflow_backend"] = p.get(
                    "workflow_backend",
                    self._pycmor_cfg("pipeline_workflow_orchestrator"),
                )
    ...
    def _post_init_create_rules(self):
        ...
        self._post_init_attach_pycmor_config_rules()

    def _post_init_attach_pycmor_config_rules(self):
        for rule in self.rules:
    ...
    def from_dict(cls, data):
        ...
        for rule in data.get("rules", []):
            ...
            instance._post_init_attach_pycmor_config_rules()
        ...
        for pipeline in data.get("pipelines", []):
            pipeline["workflow_backend"] = pipeline.get(
                "workflow_backend",
                instance._pycmor_cfg("pipeline_workflow_orchestrator"),
            )
    ...
    def check_rules_for_table(self, table_name):
        ...
        for cmor_variable in self._cmor_tables[table_name]["variable_entry"]:
            if self._rule_for_cmor_variable(cmor_variable) == []:
                ...
                elif self._pycmor_cfg.get("warn_on_no_rule", True):
                    # FIXME(PG): This should be handled by the logger automatically
                    if not self._pycmor_cfg.get("quiet", True):
    ...
    def process(self, parallel=None):
        ...
        if parallel is None:
            parallel = self._pycmor_cfg.get("parallel", True)
        if parallel:
            logger.debug("Parallel processing...")
            # FIXME(PG): This is mixed up, hard-coding to prefect for now...
            workflow_backend = self._pycmor_cfg.get("pipeline_orchestrator", "prefect")
