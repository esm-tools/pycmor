...
python tools/sanity_check/build_html_report.py \
    --jsonl tools/sanity_check/reports/cli62_full.jsonl \
    --out-dir tools/sanity_check/reports/cli62_full_html

cd tools/sanity_check/reports
tar -czf cli62_full_html.tar.gz cli62_full_html
ls -lh cli62_full_html.tar.gz
